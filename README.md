...
This repo declares the `frasermolyneux-copilot` MCP server in `.github/copilot/mcp_config.json` and builds it during `copilot-setup-steps.yml` (pinned to `frasermolyneux/.github-copilot@v0.1.0`). For client wire-up instructions (VS Code, GitHub Copilot coding agent, Copilot CLI, Claude Desktop, etc.) and the full tool contract, see `.github-copilot/mcp-server/README.md` in the shared config repo.
