...
- **Local dev loop**: `dotnet build MX.TravelItinerary.slnx` then `dotnet run --project src/MX.TravelItinerary.Web/MX.TravelItinerary.Web.csproj`. Ensure `Storage:TableEndpoint` and Entra settings are present so `DefaultAzureCredential` works.
